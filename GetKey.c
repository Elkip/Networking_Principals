...
int GetKey(struct P2Key req) {
    ...
    struct sockaddr_in keymanageAddr;   //Address of keymanager
    struct Key2P info;                  //structure containing principal pub key
    int portno = PORT;                  //Port
    int sock;                           //UDP Socket
    ...
    if (sendto(sock, &req, sizeof (struct P2Key), 0,
            (struct sockaddr *) &keymanageAddr, sizeof (struct sockaddr)) < 0)
        DieWithError("Failed to request client key");

    //receive response from key manager
    if (recv(sock, &info, sizeof (struct Key2P), 0) < 0)
